package com.silin.utils
{
	...
	import com.silin.filters.ColorAdjust;
	import flash.text.*;
	import flash.utils.*;
	/**
	 * статический класс для показа алерта<br>
	 * и/или блокировки интерактивонсти заблуренным скриншотом<br>
	 * нужен вызов Alert.register - привязка к контейнеру<br>
	 * все параметры должны быть установлены до вызова Alert.register()<br>
	 * @example 
	 * <a href="http://www.silin.fatal.ru/AS3/utils/alert">silin.fatal.ru</a>
	...
	public class Alert {
		 ...
		 * @param	_host	
		 */ 
		public static function register(_host:DisplayObjectContainer)	:void
		{
			if (Alert._host) 
			{
		...
		public static function clear(evnt:MouseEvent=null):void	
		{
			...
			if (_body.parent) 	_host.removeChild(_body);
			_messageTF.text = "";
		...
		
		{
			...
			//var cX:int = (_host.stage.stageWidth - _okBut.width) / 2;
			var cX:int = _host.stage.stageWidth / 2;
			var cY:int = (_host.stage.stageHeight - _okBut.height) / 2;
		...
		public static function block():void
		{
			...
			var bmd:BitmapData = new BitmapData(_host.stage.stageWidth, _host.stage.stageHeight, true, 0);
			bmd.draw(_host);
